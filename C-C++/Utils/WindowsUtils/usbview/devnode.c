...
DriverNameToDeviceInst(
    ...
    *pDevInfo = INVALID_HANDLE_VALUE;
    ...
    //
    deviceInfo = SetupDiGetClassDevsW(NULL,
        NULL,
        NULL,
...
DriverNameToDeviceProperties(
    ...
    // Get device instance
    status = DriverNameToDeviceInst(pDriverName, cbDriverName, &deviceInfo, &deviceInfoData);
    if (status == FALSE) {
        goto Done;
    }

    len = 0;
    status = SetupDiGetDeviceInstanceIdW(deviceInfo,
        &deviceInfoData,
        NULL,
        0,
        &len);
    ...
    len++;
    DevProps->DeviceId = ALLOC(len * sizeof(WCHAR));

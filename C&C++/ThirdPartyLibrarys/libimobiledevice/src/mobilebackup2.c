...
LIBIMOBILEDEVICE_API mobilebackup2_error_t mobilebackup2_receive_raw2(mobilebackup2_client_t client, char *data, uint32_t length, uint32_t *bytes)
{
	if (!client || !client->parent || !data || (length == 0) || !bytes)
		return MOBILEBACKUP2_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API mobilebackup2_error_t mobilebackup2_receive_raw(mobilebackup2_client_t client, char *data, uint32_t length, uint32_t *bytes)
{
	...
	*bytes = 0;
	...
	int bytes_loc = 0;
	uint32_t received = 0;
	service_error_t recvErr = SERVICE_E_UNKNOWN_ERROR;
	do {
		bytes_loc = 0;
		recvErr = service_receive(raw, data+received, length-received, (uint32_t*)&bytes_loc);
		if (bytes_loc <= 0) break;
		received += bytes_loc;
	} while (received < length);
	if (received > 0) {

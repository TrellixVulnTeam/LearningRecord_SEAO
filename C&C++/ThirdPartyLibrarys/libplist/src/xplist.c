...
static size_t dtostr(char *buf, size_t bufsize, double realval)
{
    ...
    f = modf(f, &ip);
#ifdef WIN32
    len = _snprintf(buf, bufsize, "%s%" PRIi64, ((f < 0) && (ip >= 0)) ? "-" : "", (int64_t)ip);
#else
...
static void node_to_xml(node_t* node, bytearray_t **outbuf, uint32_t depth)
{
    ...
    case PLIST_UINT:
        ...
        if (node_data->length == 16) {
            val_len = _snprintf(val, 64, "%" PRIu64, node_data->intval);
        } else {
            val_len = _snprintf(val, 64, "%" PRIi64, node_data->intval);
        }
#else
        if (node_data->length == 16) {
